...
(* Needs to be duplicated for signature because of value restriction? *)
let intf_args = Deriving.Args.(empty +> arg "with_path" (Ast_pattern.ebool __))
(* TODO: add arg_default to ppxlib? *)

(* Allow args in signature for backwards-compatibility, even though they have no effect. *)
let intf_generator = Deriving.Generator.V2.make intf_args (fun ~ctxt:_ (_, type_decls) _ ->
